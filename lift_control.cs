...
public class MyLinkList<T>//链表类，包含链表定义及基本操作方法  
{
    ...
    public void SetData(int  pos, T  per)
   {
    ...
    public Node<T> Index( int pos) {//定位函数，返回第pos个结点的地址
	 if (pos<0||pos> Size()-1)
	 {
...
//enum state { WAITING, UP, DOWN, OPENING, CLOSEING, OPENED, CLOSED, OUT, IN, IS_STOP, DECELERATE };
public class LiftControl
{
    ...
    {
        OutList.Append(p);
        //cout << "第" << p.ID << "号乘客在" << p.GetStartFloor() << "层发出请求" << endl;
    ...
    public void getfinal()
    {
            ...
            if ((StateBeforeStop == "UP") && (Floor != 9))
            {
                ...
                {//电梯里有人
                    for (int i = 0; i < InList.Size(); i++)
                    {
                        ...
                        if (distance >= m && distance > 0)
                        {//在向上方向上的电梯内的最远请求楼层
                            m = distance;
                            upnearIn = InList.GetData(i).Endfloor;
                        }
                    }
                }
                ...
                {//等待队列有人
                    ...
                    for (int i = 0; i < OutList.Size(); i++)
                    {
                        ...
                        if (distance >= m1 && distance > 0)
                        {//在向上方向上的电梯外的最远请求楼层
                            m1 = distance;
                            upnearOut = OutList.GetData(i).Startfloor;
                        }
                    }
                }
                if (upnearIn == 0 && upnearOut == 0) State ="DOWN";//说明再往上没有任务,下楼

                if (upnearIn <= upnearOut) farest = upnearOut;
                else farest = upnearIn;
            }
            if (((StateBeforeStop == "DOWN") && (Floor != 1)))
            {
                int downnearIn=0, downnearOut=0;
                int m = 0;
                if (InList.IsEmpty() == false)//电梯里有人
                {
                    for (int i = 0; i < InList.Size(); i++)
                    {
                        ...
                        {//在向下方向上的电梯内的最远请求楼层
                            m = distance;
                            downnearIn = InList.GetData(i).Endfloor;
                        }
                    }
                }
                if (OutList.IsEmpty() == false)
                {//等待队列有人
                    ...
                    for (int i = 0; i < OutList.Size(); i++)
                    {
                        ...
                        {//在向下方向上的电梯外的最远请求楼层
                            m = distance;
                            downnearOut = OutList.GetData(i).Startfloor;
                        }
                    }
                }
                if (downnearIn == 0 && downnearOut == 0) State = "UP";//说明再往下没有任务,上楼

                if (downnearIn <= downnearOut) farest = downnearIn;
                else farest = downnearOut;
            }
        }
        ...
        if (State == "DOWN")
        {
            ...
            if (downnearIn <= downnearOut) farest = downnearOut;
            else farest = downnearIn;
        }
        Go(farest);
    }
    ...
    public void Judge_tolerant()
    {
        for (int i = 0; i < OutList.Size(); i++)
        {
            ...
            p.tolerant++;
            OutList.SetData(i, p);
            if (OutList.GetData(i).tolerant > 500 && State != "OPENING" && State != "OPENED")
            {
...
public class lift_control : MonoBehaviour
{
    ...
    public Text text;
    private int[] queue = new int[9];
    public GameObject up_people;
    public GameObject down_people;
    public GameObject[,] allpeople = new GameObject[9, 13];
    private int id = 1;
    LiftControl mylift;
    ...
    void FixedUpdate()
    {
        if (mylift.OutList.hasdelete == true)
        {
